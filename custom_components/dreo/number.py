...
def get_entries(pydreo_devices: list[PyDreoBaseDevice]) -> list[DreoNumberHA]:
    ...
    for pydreo_device in pydreo_devices:
        ...
        for number_definition in NUMBERS:
            _LOGGER.debug(
                "Number:get_entries: checking attribute: %s on %s",
                number_definition.attr_name,
                pydreo_device.name,
            )
            ...
            if pydreo_device.is_feature_supported(number_definition.attr_name):
                if number_definition.key in number_keys:
                    _LOGGER.error(
                        "Number:get_entries: Duplicate number key %s",
                        number_definition.key,
                    )
                    continue

                _LOGGER.debug(
                    "Number:get_entries: Adding Number %s for %s",
                    number_definition.key,
                    number_definition.attr_name,
                )
                ...
                if device_range is not None and isinstance(device_range, tuple):
                    ...
                    number_ha_collection.append(
                        DreoNumberHA(pydreo_device, number_definition)
                    )

    return number_ha_collection


def get_device_range(
    device: PyDreoBaseDevice, number_definition: DreoNumberEntityDescription
) -> tuple | None:
    ...
    if range_from_device is not None:
        _LOGGER.debug(
            "Number:get_device_range: range %s from device is %s",
            range_name,
            range_from_device,
        )
        return range_from_device

    range_from_device_definition = getattr(
        device.device_definition.device_ranges, range_name, None
    )
    if range_from_device_definition is not None:
        _LOGGER.debug(
            "Number:get_device_range: range %s from device definition is %s",
            range_name,
...
class DreoNumberHA(DreoBaseDeviceHA, NumberEntity):  # pylint: disable=abstract-method
    ...
    def __init__(
        self, pyDreoDevice: PyDreoBaseDevice, description: DreoNumberEntityDescription
    ) -> None:
