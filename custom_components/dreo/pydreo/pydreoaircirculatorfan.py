...
class PyDreoAirCirculatorFan(PyDreoBaseDevice):
    ...
    def handle_server_update(self, message):
        ...
        valWindMode = self.get_server_update_key_value(message, AIR_CIRCULATOR_WIND_MODE_KEY)
    ...
    def set_horizontal_oscillation_angle(self, angle: int) -> None:
        ...
        if self_horizontal_oscillation_angles is not None and angle in self._horizontal_oscillation_angles.values():
            self._send_command(HORIZONTAL_OSCILLATION_ANGLE_KEY, angle)
        else:
            _LOGGER.error("Angle %s is not supported for this device, supported angles are: %s", angle, self._horizontal_oscillation_angles)
    ...
    def set_vertical_oscillation_angle(self, angle: int) -> None:
        ...
        if self_vertical_oscillation_angles is not None and angle in self._vertical_oscillation_angles.values():
            self._send_command(VERTICAL_OSCILLATION_ANGLE_KEY, angle)
        else:
            _LOGGER.error("Angle %s is not supported for this device, supported angles are: %s", angle, self._vertical_oscillation_angles)
            return
