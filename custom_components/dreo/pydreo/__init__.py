...
class PyDreo:  # pylint: disable=function-redefined
        ...
        for dev in devices:
            ...
            try:
                ...
                if model is not None:
                    ...
                    for prefix in SUPPORTED_MODEL_PREFIXES:
                        if model[: len(prefix) :] == prefix:
                            model_prefix = prefix
                            _LOGGER.debug(
                                "Prefix %s assigned from model %s", model_prefix, model
                            )
                            break
                    ...
                    if model in SUPPORTED_DEVICES:
                        ...
                        _LOGGER.debug(
                            "Device %s found! via prefix %s", model, model_prefix
                        )
                ...
                if device_details is not None:
                    device_class = _DREO_DEVICE_TYPE_TO_CLASS.get(
                        device_details.device_type, None
                    )
                else:
                    device_details = DreoDeviceDetails(
                        device_type=DreoDeviceType.UNKNOWN
                    )
    ...
    def get_device_setting(
        self, device: PyDreoBaseDevice, setting: DreoDeviceSetting
    ) -> bool | int:
        """Get a device setting from the API."""
        _LOGGER.debug(
            "get_device_setting: %s(%s), enabled: %s",
            device.name,
            setting,
            self.enabled,
        )
        ...
        response, _ = self.call_dreo_api(
            ...
            {DEVICESN_KEY: device.serial_number, DREO_API_SETTING_DATA_KEY: setting},
        )
        ...
        if response and Helpers.code_check(response):
            if DATA_KEY in response:
                ...
                if DREO_API_SETTING_DATA_VALUE in data_node:
                    ...
                    _LOGGER.error(
                        "%s key not found in returned data. %s",
    ...
    def set_device_setting(
        self, device: PyDreoBaseDevice, setting: DreoDeviceSetting, value: bool | int
    ) -> None:
        """Get a device setting from the API."""
        _LOGGER.debug(
            "set_device_setting: %s(%s=%s), enabled: %s",
            device.name,
            setting,
            value,
            self.enabled,
