...
class CommandTransport:
    ...
    async def _start_websocket(self) -> None:
        ...
        async for ws in websockets.connect(url):
            ...
            if not self._auto_reconnect:
                _LOGGER.error(
                    "WebSocket appears closed.  Not Reconnecting.  Restart HA to reconnect."
                )
                break  # This break causes us not to connect
            else:
                continue
    ...
    async def _ws_handler(self, ws):
        ...
        ping_task = asyncio.create_task(self._ws_ping_handler(ws))
            ...
            [consumer_task, ping_task], return_when=asyncio.FIRST_COMPLETED
        )
    ...
    async def _ws_consumer_handler(self, ws):
        ...
        try:
            ...
            _LOGGER.debug(
                "CommandTransport::_ws_consumer_handler - WebSocket appears closed."
            )
    ...
    async def _ws_ping_handler(self, ws):
        ...
        while True:
            try:
                ...
                if self._signal_close:
                    _LOGGER.debug(
                        "CommandTransport::_ws_ping_handler - Closing WebSocket"
                    )
                    await ws.close()
                ...
                _LOGGER.info(
                    "Dreo WebSocket Cancelled - Unless intended, will reconnect"
                )
                break
        ...
        async def send_internal() -> None:
            ...
            while retry_count < MAX_RETRY_COUNT:
                try:
                    ...
                    retry_count += 1
                    _LOGGER.error(
                        "Error sending command. Retrying in %s seconds. Retry count: %s",
                        RETRY_DELAY,
                        retry_count,
                    )
