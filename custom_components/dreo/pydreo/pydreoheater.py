...
class PyDreoHeater(PyDreoBaseDevice):
    ...
    def heat_range(self):
        """Get the heat range"""
        return self._device_definition.speed_range[HEAT_RANGE]
    ...
    def htalevel(self, htalevel : int) :
        ...
        if (htalevel < self._device_definition.speed_range[HEAT_RANGE][0] or 
            htalevel > self._device_definition.speed_range[HEAT_RANGE][1]):
            _LOGGER.error("Heat level %s is not in the acceptable range: %s",
                            htalevel,
                            self._device_definition.speed_range[HEAT_RANGE])
            return
    ...
    def ecolevel_range(self):
        ...
        return self._device_definition.speed_range[ECOLEVEL_RANGE]
    ...
    def ecolevel(self, ecolevel : int):
        ...
        # TODO: Change to in range check
        if ecolevel < self._device_definition.speed_range[ECOLEVEL_RANGE][0] or ecolevel > self._device_definition.speed_range[ECOLEVEL_RANGE][1]:
            _LOGGER.error("Target Temperature %s is not in the acceptable range: %s",
                            ecolevel,
                            self._device_definition.speed_range[ECOLEVEL_RANGE])
            return
