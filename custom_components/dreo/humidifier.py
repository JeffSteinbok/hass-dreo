...
def get_entries(pydreo_devices: list[PyDreoBaseDevice]) -> list[DreoHumidifierHA]:
    ...
    for pydreo_device in pydreo_devices:
        if pydreo_device.type == DreoDeviceType.HUMIDIFIER:
            _LOGGER.debug(
                "Humidifier:get_entries: Found a %s - %s",
                pydreo_device.type,
                pydreo_device.name,
            )
...
async def async_setup_entry(
    ...
    _LOGGER.debug(
        "Humidifier:async_setup_entry: Adding Humidifiers (%s)",
        len(humidifier_entities_ha),
    )
...
class DreoHumidifierHA(DreoBaseDeviceHA, HumidifierEntity):
    ...
    def set_mode(self, mode: str) -> None:
        ...
        _LOGGER.debug("DreoHumidiferHA:set_mode(%s) --> %s", self.device.name, mode)
