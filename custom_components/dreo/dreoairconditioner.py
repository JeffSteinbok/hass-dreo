...
from .pydreo.pydreoairconditioner import DreoACMode, DreoACFanMode
...
class DreoAirConditionerHA(DreoBaseDeviceHA, ClimateEntity):
    ...
    def __init__(self, pyDreoDevice: PyDreoAC) -> None:
        ...
        hvac_modes_set = {
            DREO_MODE_TO_HVAC_MODE.get(DreoACMode(mode_num)) for mode_num in self.device.modes if DreoACMode(mode_num) in DREO_MODE_TO_HVAC_MODE
        }
        ...
        preset_modes_set = {
            DREO_MODE_TO_PRESET.get(DreoACMode(mode_num)) for mode_num in self.device.modes if DreoACMode(mode_num) in DREO_MODE_TO_PRESET
        }
    ...
    def fan_mode(self) -> str:
        ...
        _LOGGER.debug("fan_mode: fan_mode(%s): %s (device.fan_mode: %s)", self.device.name, ha_fan_mode, self.device.fan_mode)
        return ha_fan_mode

    def set_fan_mode(self, fan_mode: str) -> None:
        """Set new target fan mode."""
        _LOGGER.debug("set_fan_mode: set_fan_mode(%s) --> %s", self.device.name, fan_mode)
        dreo_fan_mode = HA_FAN_MODE_TO_DREO.get(fan_mode)
        if dreo_fan_mode is not None:
            self.device.fan_mode = dreo_fan_mode
            self._attr_fan_mode = fan_mode
        else:
            _LOGGER.warning("set_fan_mode: set_fan_mode(%s) invalid fan mode: %s", self.device.name, fan_mode)
    ...
    def preset_mode(self) -> str | None:
        ...
        _LOGGER.debug("preset_mode: preset_mode(%s): %s (device.mode: %s)", self.device.name, preset, self.device.mode)
        return preset
    ...
    def set_preset_mode(self, preset_mode: str) -> None:
        ...
        if dreo_mode is not None:
            ...
            _LOGGER.warning(
                "set_preset_mode: set_preset_mode(%s) invalid combination: hvac=%s, preset=%s", self.device.name, current_hvac_mode, preset_mode
            )
    ...
    def supported_features(self) -> int:
        ...
        _LOGGER.debug("supported_features: supported_features(%s): %s (device.mode: %s)", self, supported_features, self.device.mode)
    ...
    def panel_sound(self, panel_sound: bool) -> None:
        ...
        _LOGGER.debug("panel_sound: :panel_sound(%s) --> %s", self.device.name, panel_sound)
    ...
    def set_temperature(self, **kwargs: Any) -> None:
        ...
        _LOGGER.debug(
            "set_temperature: :set_temperature(%s) %s --> %s", self.device.name, self._attr_target_temperature, self.device.target_temperature
        )
        self._attr_target_temperature = self.device.target_temperature
        self.schedule_update_ha_state()
    ...
    def set_humidity(self, humidity: int) -> None:
        ...
        _LOGGER.debug("set_humidity: :set_humidity(%s) %s --> %s", self.device.name, self._attr_target_humidity, humidity)
    ...
    def hvac_mode(self):
        ...
        self._attr_hvac_mode = DREO_MODE_TO_HVAC_MODE.get(DreoACMode(self.device.mode), HVACMode.OFF) if self.device.poweron else HVACMode.OFF
        _LOGGER.debug("hvac_mode: hvac_mode(%s): %s (device.mode: %s)", self.device.name, self._attr_hvac_mode, self.device.mode)
    ...
    def set_swing_mode(self, swing_mode: str) -> None:
        ...
        _LOGGER.debug("set_swing_mode: set_swing_mode(%s) -> %s", self.device.name, swing_mode)
