...
class TestDreoDeHumidifier(IntegrationTestBase):
    ...
    def test_HDH005S_DH515S(self):  # pylint: disable=invalid-name
        """Load DR-HDH005S (DH515S series) dehumidifier from real diagnostics and test."""
        with patch(PATCH_SCHEDULE_UPDATE_HA_STATE):

            self.get_devices_file_name = "get_devices_HDH005S_DH515S.json"
            self.pydreo_manager.load_devices()
            assert len(self.pydreo_manager.devices) == 1

            pydreo_dehumidifier : PyDreoDehumidifier = self.pydreo_manager.devices[0]
            assert pydreo_dehumidifier.type == 'Dehumidifier'
            assert pydreo_dehumidifier.model == "DR-HDH005S"

            ha_humidifier = humidifier.DreoDehumidifierHA(pydreo_dehumidifier)
            assert ha_humidifier.is_on is True
            assert ha_humidifier.current_humidity == 60
            assert ha_humidifier.target_humidity == 50
            assert ha_humidifier.unique_id is not None
            assert ha_humidifier.name is not None

            # Verify fan speed (windlevel=3 → High)
            assert pydreo_dehumidifier.wind_level == 3
            assert pydreo_dehumidifier.preset_mode == "High"

            # Verify auto mode is on
            assert pydreo_dehumidifier.auto_mode is True

            # Verify operating mode (mode=1 → Auto)
            assert pydreo_dehumidifier.mode == "Auto"
            ...
            # Check to see what numbers are added
            numbers = number.get_entries([pydreo_dehumidifier])
            self.verify_expected_entities(numbers, ["Target Humidity"])
    ...
    def test_HDH005S(self):  # pylint: disable=invalid-name
        ...
        with patch(PATCH_SCHEDULE_UPDATE_HA_STATE):
            ...
            # Find the Target Humidity number and verify its value
            target_humidity_number = None
            for n in numbers:
                if n.entity_description.key == "Target Humidity":
                    target_humidity_number = n
